...
assert_text_scroll 'تسجيل الدخول مطلوب للمزامنة' auth-required
tap_text_scroll 'مركز المزامنة'
assert_text_scroll 'Backend ANDAK متصل' backend-online
assert_text_scroll 'Live Snapshot' live-snapshot
assert_text_scroll 'الجلسة: غير مسجل الدخول' session-none
assert_text_scroll 'تحديث بيانات الخادم' refresh-button

# Create a local operation and verify it remains blocked without supplier auth.
...
assert_text_scroll 'محجوبة — يلزم تسجيل دخول المورد' blocked-auth
...
echo SIGNED_GATE10_RUNTIME_PASS | tee "$E/result.txt"
