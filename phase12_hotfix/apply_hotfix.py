...
# Keep the recovery UI dependency-free so it compiles without extra Compose imports.
...
'''    var seeded by remember { mutableStateOf(false) }\n    var user by remember { mutableStateOf<UserEntity?>(null) }\n    var startupError by remember { mutableStateOf<String?>(null) }\n\n    LaunchedEffect(Unit) {\n        try {\n            container.seedIfNeeded()\n            seeded = true\n        } catch (t: Throwable) {\n            startupError = (t::class.simpleName ?: "Error") + ": " + (t.message ?: "بدون تفاصيل")\n        }\n    }\n\n    if (startupError != null) {\n        androidx.compose.material3.Text(\n            "تعذر فتح قاعدة البيانات - لم يتم حذف بياناتك\\n" +\n                (startupError ?: "") +\n                "\\nأرسل صورة هذه الرسالة لإصلاح الترحيل بدون حذف البيانات."\n        )\n    } else if (!seeded) {\n        androidx.compose.material3.Text("جاري تهيئة قاعدة البيانات...")\n    } else if (user == null) {''')
