...
new_totals = '''        val totalOriginal = prepared.sumOf { it.request.amountOriginal }
        ...
        val receiptNo = numbering.nextDocumentNo("RCPT", receiptDate)
'''
text = replace_once(text, old_totals, new_totals, "receipt preflight before mutation")
...
/** Separate receipt-posting atomicity hardening. Not the Final Audit AE-ACC-009 fix. */
...
class CustomerReceiptAtomicityPolicyTest {
    @Test
    fun allowsReceiptMutationInsideActiveTransaction() {
        CustomerReceiptAtomicityPolicy.requireActiveTransaction(true)
    }

    @Test
    fun rejectsReceiptMutationWithoutActiveTransaction() {
...
print("customer receipt atomicity hardening applied (separate handoff)")
