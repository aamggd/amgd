...
class ProductionMathTest {
    ...
    @Test
    fun `packaging quantities follow planned finished pieces while raw batch inputs stay fixed`() {
        assertEquals(2.5, ProductionMath.orderComponentQuantity(2.5, 375.0, "RM-BORIC"), 1e-9)
        assertEquals(375.0, ProductionMath.orderComponentQuantity(372.0, 375.0, "PK-BOTTLE-60"), 1e-9)
        assertEquals(375.0, ProductionMath.orderComponentQuantity(372.0, 375.0, "PK-LABEL-60"), 1e-9)
        assertEquals(16.0, ProductionMath.orderComponentQuantity(15.0, 375.0, "PK-PACK"), 1e-9)
        assertEquals(16.0, ProductionMath.orderComponentQuantity(15.0, 375.0, "PK-PACK-LABEL"), 1e-9)
    }
    ...
    @Test
    fun trackingCompletenessCanFilterInvalidLotsWithoutRejectingValidLots() {
        assertTrue(ProductionMath.isIssueLotTrackingComplete(true, false, "EGG-LOT-001", null))
        assertFalse(ProductionMath.isIssueLotTrackingComplete(true, false, null, null))
        assertFalse(ProductionMath.isIssueLotTrackingComplete(false, true, null, null))
        assertTrue(ProductionMath.isIssueLotTrackingComplete(false, true, null, 1_800_000_000_000L))
    }
