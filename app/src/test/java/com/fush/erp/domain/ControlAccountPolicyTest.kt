...
class ControlAccountPolicyTest {
    ...
    @Test(expected = IllegalArgumentException::class)
    fun inventoryControlRejectsManualJournal() {
        ControlAccountPolicy.requireManualPostingAllowed("1200")
    }

    @Test(expected = IllegalArgumentException::class)
    fun inventoryControlRejectsGenericVoucherBypass() {
        ControlAccountPolicy.requireGenericVoucherAllowed("1200")
    }

    @Test
    fun ordinaryExpenseAccount_isAllowed() {
        ControlAccountPolicy.requireManualPostingAllowed("6500")
        ControlAccountPolicy.requireGenericVoucherAllowed("6500")
        assertTrue(true)
    }
