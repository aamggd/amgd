...
class InventoryReportMathTest {
    ...
    fun expiry_classifies_expired_near_and_later_lots() {
        ...
        assertEquals("قريب الانتهاء", InventoryReportMath.expiry(expiry(asOf + 20L * day), asOf, 30).status)
        assertEquals("قريب الانتهاء", InventoryReportMath.expiry(expiry(asOf + 60L * day), asOf, 60).status)
        assertEquals("ساري الصلاحية", InventoryReportMath.expiry(expiry(asOf + 120L * day), asOf).status)
    }
