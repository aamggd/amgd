...
class WarehouseTransferMathTest {
    ...
    @Test
    fun backdatedOutflowBeforeFirstReceiptHasZeroSafeQuantity() {
        val points = listOf(
            WarehouseTransferBalancePoint(10L, 100.0)
        )
        assertEquals(0.0, WarehouseTransferMath.minimumAvailableFrom(points, 5L), 0.000001)
    }

    @Test
    fun backdatedOutflowIsLimitedByLowestLaterBalance() {
        val points = listOf(
            WarehouseTransferBalancePoint(1L, 100.0),
            WarehouseTransferBalancePoint(10L, -80.0),
            WarehouseTransferBalancePoint(20L, 50.0)
        )
        assertEquals(20.0, WarehouseTransferMath.minimumAvailableFrom(points, 5L), 0.000001)
        WarehouseTransferMath.validateHistoricalQuantity(20.0, 20.0)
        assertThrows(IllegalArgumentException::class.java) {
            WarehouseTransferMath.validateHistoricalQuantity(20.01, 20.0)
        }
    }
