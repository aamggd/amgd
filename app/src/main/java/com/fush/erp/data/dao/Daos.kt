...
interface UserDao {
    ...
    @Query("SELECT COUNT(*) FROM users WHERE role = :roleCode")
    suspend fun userCountForRole(roleCode: String): Int
...
interface AccountDao {
    ...
    suspend fun allRows(): List<AccountEntity>
...
interface UnitDao {
    ...
    suspend fun allRows(): List<UnitEntity>
...
interface WarehouseDao {
    ...
    suspend fun allRows(): List<WarehouseEntity>
...
interface ItemDao {
    ...
    @Query("SELECT * FROM items ORDER BY nameAr")
    suspend fun allRows(): List<ItemEntity>
...
interface JournalDao {
    ...
    @Insert(onConflict = OnConflictStrategy.ABORT)
    suspend fun insertLinesRaw(rows: List<JournalLineEntity>)

    @Query("SELECT sourceType FROM journal_entries WHERE id = :entryId LIMIT 1")
    suspend fun sourceTypeForEntry(entryId: Long): String?

    @Query("UPDATE journal_entries SET status = 'POSTED' WHERE id = :entryId AND status = 'STAGING'")
    suspend fun transitionStagingToPosted(entryId: Long): Int

    /**
     * v190 cloud-only finalization for historical TREASURY_* hydration. The status and real
     * sourceType are restored in one SQLite UPDATE so the period guard can distinguish remote
     * replication from a new local posting.
     */
    @Query("""
        UPDATE journal_entries
        SET status = 'POSTED', sourceType = :originalSourceType, sourceId = :sourceId
        WHERE id = :entryId
          AND status = 'STAGING'
          AND UPPER(TRIM(sourceType)) = 'CLOUD_LEGACY_TREASURY_HYDRATION'
          AND LOWER(TRIM(sourceId)) LIKE 'cloud-legacy:%'
    """)
    suspend fun finalizeLegacyCloudHydrationToPosted(
        entryId: Long,
    ...
    /**
     * v196 cloud-only finalization for any already-posted journal hydrated from the cloud.
     * The internal alias exists only while STAGING and can never remain POSTED.
     */
    @Query("""
        UPDATE journal_entries
        SET status = 'POSTED', sourceType = :originalSourceType, sourceId = :sourceId
        WHERE id = :entryId
          AND status = 'STAGING'
          AND UPPER(TRIM(sourceType)) = 'CLOUD_POSTED_JOURNAL_HYDRATION'
          AND LOWER(TRIM(sourceId)) LIKE 'cloud-hydration:%'
    """)
    suspend fun finalizeCloudHydrationToPosted(
        entryId: Long,
        originalSourceType: String,
        sourceId: String?,
    ): Int
    ...
    suspend fun insertLines(rows: List<JournalLineEntity>) {
        require(rows.isNotEmpty()) { "القيد يجب أن يحتوي سطوراً" }
        val entryIds = rows.map { it.entryId }.distinct()
        require(entryIds.size == 1) { "يجب إدخال سطور قيد واحد في دفعة ذرية واحدة" }
        val entryId = entryIds.single()
        val sourceType = requireNotNull(sourceTypeForEntry(entryId)) { "رأس القيد غير موجود" }
        insertLinesRaw(rows)
        if (!sourceType.equals("MANUAL", ignoreCase = true)) {
            val transitioned = transitionStagingToPosted(entryId)
            require(transitioned == 1) { "فشل ترحيل القيد من STAGING إلى POSTED" }
        }
    }
    ...
    @Query("SELECT * FROM journal_entries WHERE entryNo = :entryNo COLLATE NOCASE LIMIT 1")
    suspend fun byEntryNo(entryNo: String): JournalEntryEntity?

    @Query("SELECT * FROM journal_entries WHERE status = 'POSTED' ORDER BY entryDate, id")
    suspend fun allPostedForCloudSync(): List<JournalEntryEntity>

    @Update
    suspend fun updateEntry(row: JournalEntryEntity)

    @Query("DELETE FROM journal_lines WHERE entryId = :entryId")
    suspend fun deleteLinesForEntry(entryId: Long): Int
    ...
    @Query("SELECT * FROM journal_entries WHERE status = 'POSTED' AND UPPER(TRIM(sourceType)) = UPPER(TRIM(:sourceType)) AND TRIM(sourceId) = TRIM(:sourceId) ORDER BY id DESC LIMIT 1")
    suspend fun bySourceNormalized(sourceType: String, sourceId: String): JournalEntryEntity?
