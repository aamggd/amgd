...
import com.fush.erp.domain.AccountingPeriodPostingPolicy
...
interface AccountingDao {
    ...
    @Query("SELECT * FROM treasury_accounts WHERE code = :code COLLATE NOCASE LIMIT 1")
    suspend fun treasuryByCode(code: String): TreasuryAccountEntity?
    ...
    @Update
    suspend fun updateTreasury(row: TreasuryAccountEntity)

    @Query("SELECT * FROM treasury_accounts WHERE groupCode = :groupCode ORDER BY currencyCode, id")
    suspend fun treasuriesByGroupCode(groupCode: String): List<TreasuryAccountEntity>
    ...
    @Query("SELECT * FROM treasury_accounts ORDER BY kind, nameAr")
    fun observeAllTreasury(): Flow<List<TreasuryAccountEntity>>
    ...
    suspend fun periodForDateOrNull(date: Long): AccountingPeriodEntity?

    suspend fun periodForDate(date: Long): AccountingPeriodEntity =
        AccountingPeriodPostingPolicy.requirePeriod(periodForDateOrNull(date), date)
    ...
    @Query("""
        SELECT t.id AS id, t.code AS code, t.groupCode AS groupCode, t.nameAr AS nameAr, t.kind AS kind,
        ...
        WHERE t.isActive = 1
        GROUP BY t.id, t.code, t.groupCode, t.nameAr, t.kind, t.accountId, t.currencyCode, t.bankName, t.accountNumber
    ...
    @Query("""
        SELECT COALESCE(SUM(balanceBase), 0)
        FROM (
            SELECT t.id AS treasuryId,
                   COALESCE(SUM(CASE WHEN je.status='POSTED' AND je.entryDate <= :asOf THEN jl.debit - jl.credit ELSE 0 END), 0) AS balanceBase
            FROM treasury_accounts t
            LEFT JOIN journal_lines jl ON jl.accountId = t.accountId
            LEFT JOIN journal_entries je ON je.id = jl.entryId
            WHERE t.isActive = 1
    ...
    """)
    suspend fun treasuryTotalBookBalance(asOf: Long): Double
