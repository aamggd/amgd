...
interface SalesRepresentativeDao {
    ...
    @Query("SELECT * FROM sales_representatives ORDER BY code COLLATE NOCASE, id")
    suspend fun allForCloudReferenceSync(): List<SalesRepresentativeEntity>

    @Query("SELECT * FROM sales_representatives WHERE id=:id LIMIT 1")
    suspend fun byId(id: Long): SalesRepresentativeEntity?

    @Query("SELECT * FROM sales_representatives WHERE code=:code COLLATE NOCASE LIMIT 1")
    suspend fun byCode(code: String): SalesRepresentativeEntity?
