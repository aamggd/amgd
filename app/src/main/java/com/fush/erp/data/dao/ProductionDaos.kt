...
interface RecipeDao {
    ...
    @Query("SELECT * FROM recipes ORDER BY code, versionNo")
    suspend fun allRecipesForCloudSync(): List<RecipeEntity>

    @Query("SELECT * FROM recipes WHERE code = :code AND versionNo = :versionNo LIMIT 1")
    suspend fun byCodeVersion(code: String, versionNo: Int): RecipeEntity?
}
...
interface ProductionDao {
    ...
    @Query("SELECT * FROM production_orders ORDER BY plannedDate, id")
    suspend fun allOrdersForCloudSync(): List<ProductionOrderEntity>

    @Query("SELECT * FROM production_orders WHERE orderNo = :orderNo LIMIT 1")
    suspend fun orderByNo(orderNo: String): ProductionOrderEntity?

    @Query("SELECT * FROM production_batches ORDER BY manufactureDate, id")
    suspend fun allBatchesForCloudSync(): List<ProductionBatchEntity>

    @Query("SELECT * FROM production_batches WHERE batchNo = :batchNo LIMIT 1")
    suspend fun batchByNo(batchNo: String): ProductionBatchEntity?

    @Query("SELECT * FROM production_issues ORDER BY issueDate, id")
    suspend fun allIssuesForCloudSync(): List<ProductionIssueEntity>
