...
interface PartyDao {
    ...
    @Query("SELECT * FROM party_vouchers WHERE voucherNo=:voucherNo COLLATE NOCASE LIMIT 1")
    suspend fun voucherByNo(voucherNo: String): PartyVoucherEntity?

    @Query("SELECT * FROM party_vouchers WHERE status IN ('POSTED','REVERSED') ORDER BY voucherDate, id")
    suspend fun allVouchersForCloudSync(): List<PartyVoucherEntity>
    ...
    @Query("SELECT * FROM party_attachments ORDER BY id")
    suspend fun allAttachments(): List<PartyAttachmentEntity>

    @Query("UPDATE party_attachments SET uri=:uri WHERE id=:id")
    suspend fun updateAttachmentUri(id: Long, uri: String): Int
