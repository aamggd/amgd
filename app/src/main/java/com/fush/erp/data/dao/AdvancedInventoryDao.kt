...
interface AdvancedInventoryDao {
    ...
    @Query("""
        ...
        FROM stock_movements sm
        WHERE sm.warehouseId = :warehouseId
          AND sm.movementDate <= :asOf
        GROUP BY sm.itemId, sm.lotNo, sm.expiryDate
        HAVING ABS(COALESCE(SUM(sm.quantityBase), 0)) > 0.000000001
        ORDER BY sm.itemId, CASE WHEN sm.expiryDate IS NULL THEN 1 ELSE 0 END, sm.expiryDate, sm.lotNo
    """)
    suspend fun snapshotAt(warehouseId: Long, asOf: Long): List<InventorySnapshotRow>
    ...
    @Query("""
        SELECT COALESCE(SUM(sm.quantityBase * sm.unitCostBase), 0)
        FROM stock_movements sm
        JOIN items i ON i.id = sm.itemId
        WHERE i.category = :category
    """)
    fun observeInventoryValueByCategory(category: String): Flow<Double>

    @Query("""
        SELECT ppm.itemId AS itemId,
               i.nameAr AS itemName,
               u.nameAr AS unitName,
               COALESCE((
                   SELECT SUM(CASE
                       WHEN sm.movementDate <= :at
                        AND (sm.expiryDate IS NULL OR sm.expiryDate >= :at)
                        AND COALESCE((
                            SELECT c.status
                            FROM inventory_lot_controls c
                            WHERE c.warehouseId = sm.warehouseId
                              AND c.itemId = sm.itemId
                              AND c.lotKey = COALESCE(sm.lotNo, '')
                              AND c.expiryKey = COALESCE(sm.expiryDate, -1)
                            ORDER BY c.changedAt DESC
                            LIMIT 1
                        ), 'ACCEPTED') = 'ACCEPTED'
                       THEN sm.quantityBase ELSE 0 END)
                   FROM stock_movements sm
                   WHERE sm.itemId = ppm.itemId
               ), 0) AS currentStockQtyBase,
               COALESCE(SUM(ppm.dailyUsageQtyBase), 0) AS dailyUsageQtyBase,
               CASE WHEN COALESCE(SUM(ppm.dailyUsageQtyBase), 0) > 0.000000001
                    THEN COALESCE((
                        SELECT SUM(CASE
                            WHEN sm2.movementDate <= :at
                             AND (sm2.expiryDate IS NULL OR sm2.expiryDate >= :at)
                             AND COALESCE((
                                 SELECT c2.status
                                 FROM inventory_lot_controls c2
                                 WHERE c2.warehouseId = sm2.warehouseId
                                   AND c2.itemId = sm2.itemId
                                   AND c2.lotKey = COALESCE(sm2.lotNo, '')
                                   AND c2.expiryKey = COALESCE(sm2.expiryDate, -1)
                                 ORDER BY c2.changedAt DESC
                                 LIMIT 1
                             ), 'ACCEPTED') = 'ACCEPTED'
                            THEN sm2.quantityBase ELSE 0 END)
                        FROM stock_movements sm2
                        WHERE sm2.itemId = ppm.itemId
                    ), 0) / SUM(ppm.dailyUsageQtyBase)
                    ELSE NULL END AS coverageDays
        FROM production_plan_materials ppm
        JOIN production_plans pp ON pp.id = ppm.productionPlanId
        JOIN items i ON i.id = ppm.itemId
        JOIN units u ON u.id = i.baseUnitId
        WHERE pp.planYear = :planYear
          AND pp.planMonth = :planMonth
          AND pp.status = 'APPROVED'
        GROUP BY ppm.itemId, i.nameAr, u.nameAr
        ORDER BY CASE WHEN coverageDays IS NULL THEN 1 ELSE 0 END, coverageDays, i.nameAr
    """)
    fun observeDashboardMaterialCoverage(planYear: Int, planMonth: Int, at: Long): Flow<List<DashboardMaterialCoverageRow>>
