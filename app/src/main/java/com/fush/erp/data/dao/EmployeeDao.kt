...
interface EmployeeDao {
    ...
    @Query("SELECT * FROM employees ORDER BY code COLLATE NOCASE, id")
    suspend fun allEmployeesForCloudReferenceSync(): List<EmployeeEntity>

    @Query("SELECT * FROM employees WHERE id = :id LIMIT 1")
    suspend fun employeeById(id: Long): EmployeeEntity?

    @Query("SELECT * FROM employees WHERE code = :code COLLATE NOCASE LIMIT 1")
    suspend fun employeeByCode(code: String): EmployeeEntity?
