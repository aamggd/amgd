...
interface SupplierDao {
    ...
    @Update
    suspend fun update(row: SupplierEntity)
}
...
interface ItemUnitConversionDao {
    ...
    @Query("SELECT * FROM item_unit_conversions ORDER BY itemId, unitId")
    suspend fun allRows(): List<ItemUnitConversionEntity>
...
interface PurchaseDao {
    ...
    @Query("SELECT * FROM purchase_invoices WHERE invoiceNo = :invoiceNo COLLATE NOCASE LIMIT 1")
    suspend fun invoiceByNo(invoiceNo: String): PurchaseInvoiceEntity?

    @Query("SELECT * FROM purchase_invoices ORDER BY invoiceDate, id")
    suspend fun allInvoicesForCloudSync(): List<PurchaseInvoiceEntity>

    @Query("SELECT * FROM purchase_returns WHERE id = :id LIMIT 1")
    suspend fun returnById(id: Long): PurchaseReturnEntity?

    @Query("SELECT * FROM purchase_returns WHERE returnNo = :returnNo COLLATE NOCASE LIMIT 1")
    suspend fun returnByNo(returnNo: String): PurchaseReturnEntity?

    @Query("SELECT * FROM purchase_returns ORDER BY returnDate, id")
    suspend fun allReturnsForCloudSync(): List<PurchaseReturnEntity>

    @Query("SELECT * FROM purchase_return_lines WHERE returnId = :returnId ORDER BY id")
    suspend fun returnLinesForReturn(returnId: Long): List<PurchaseReturnLineEntity>

    @Query("SELECT * FROM supplier_payments WHERE id = :id LIMIT 1")
    suspend fun supplierPaymentById(id: Long): SupplierPaymentEntity?

    @Query("SELECT * FROM supplier_payments WHERE paymentNo = :paymentNo COLLATE NOCASE LIMIT 1")
    suspend fun supplierPaymentByNo(paymentNo: String): SupplierPaymentEntity?

    @Query("SELECT * FROM supplier_payments ORDER BY paymentDate, id")
    suspend fun allSupplierPaymentsForCloudSync(): List<SupplierPaymentEntity>
    ...
    @Query("""
        SELECT COALESCE(SUM(spa.allocatedBase), 0)
        FROM supplier_payment_allocations spa
        JOIN supplier_payments sp ON sp.id = spa.paymentId
        WHERE spa.invoiceId = :invoiceId AND sp.paymentDate <= :asOf
    """)
    suspend fun paidBaseForInvoiceAsOf(invoiceId: Long, asOf: Long): Double
    ...
    @Query("""
        ...
        FROM purchase_returns
        WHERE purchaseInvoiceId = :invoiceId AND status = 'POSTED'
          AND settlementType = 'SUPPLIER_CREDIT' AND returnDate <= :asOf
    """)
    suspend fun supplierCreditReturnedBaseForInvoiceAsOf(invoiceId: Long, asOf: Long): Double
    ...
    @Query("""
        SELECT p.id AS invoiceId, p.invoiceNo AS invoiceNo, p.invoiceDate AS invoiceDate,
               ...
               p.exchangeRate AS invoiceExchangeRate, p.totalBase AS totalBase,
               COALESCE((SELECT SUM(pr.totalBase) FROM purchase_returns pr
                         WHERE pr.purchaseInvoiceId=p.id AND pr.status='POSTED'
                           AND pr.settlementType='SUPPLIER_CREDIT' AND pr.returnDate <= :asOf),0) AS supplierCreditReturnsBase,
               COALESCE((SELECT SUM(spa.allocatedBase) FROM supplier_payment_allocations spa
                         JOIN supplier_payments sp ON sp.id=spa.paymentId
                         WHERE spa.invoiceId=p.id AND sp.supplierId=p.supplierId
                           AND sp.paymentDate <= :asOf),0) AS paidBase,
               MAX(0, p.totalBase
                    - COALESCE((SELECT SUM(pr2.totalBase) FROM purchase_returns pr2
                                WHERE pr2.purchaseInvoiceId=p.id AND pr2.status='POSTED'
                                  AND pr2.settlementType='SUPPLIER_CREDIT' AND pr2.returnDate <= :asOf),0)
                    - COALESCE((SELECT SUM(spa2.allocatedBase) FROM supplier_payment_allocations spa2
                                JOIN supplier_payments sp2 ON sp2.id=spa2.paymentId
                                WHERE spa2.invoiceId=p.id AND sp2.supplierId=p.supplierId
                                  AND sp2.paymentDate <= :asOf),0)) AS outstandingBase
        FROM purchase_invoices p
        WHERE p.supplierId=:supplierId AND p.status='POSTED' AND p.paymentType='CREDIT'
          AND p.invoiceDate <= :asOf
          AND (p.totalBase
               - COALESCE((SELECT SUM(pr3.totalBase) FROM purchase_returns pr3
                           WHERE pr3.purchaseInvoiceId=p.id AND pr3.status='POSTED'
                             AND pr3.settlementType='SUPPLIER_CREDIT' AND pr3.returnDate <= :asOf),0)
               - COALESCE((SELECT SUM(spa3.allocatedBase) FROM supplier_payment_allocations spa3
                           JOIN supplier_payments sp3 ON sp3.id=spa3.paymentId
                           WHERE spa3.invoiceId=p.id AND sp3.supplierId=p.supplierId
                             AND sp3.paymentDate <= :asOf),0)) > 0.000000001
        ORDER BY COALESCE(p.dueDate,p.invoiceDate), p.invoiceDate, p.id
    """)
    suspend fun openSupplierInvoicesAsOf(supplierId: Long, asOf: Long): List<SupplierInvoicePayableRow>
...
interface StockDao {
    ...
    @Query("SELECT * FROM stock_movements ORDER BY movementDate, id")
    suspend fun allMovementsForCloudSync(): List<StockMovementEntity>
    ...
    /**
     * Positive lot balances ordered by the age of the stock in this warehouse (FIFO).
     * Shipment creation uses this to select the oldest available lot automatically.
     * Expiry date is only a deterministic tie-breaker; the primary rule is oldest stock.
     */
    @Query("""
        SELECT itemId AS itemId, lotNo AS lotNo, expiryDate AS expiryDate,
               COALESCE(SUM(quantityBase), 0) AS quantityBase,
        ...
        GROUP BY itemId, lotNo, expiryDate
        HAVING COALESCE(SUM(quantityBase), 0) > 0.000000001
        ORDER BY MIN(movementDate) ASC,
                 CASE WHEN expiryDate IS NULL THEN 1 ELSE 0 END, expiryDate ASC,
                 COALESCE(lotNo, '') ASC
    """)
    suspend fun lotBalancesFifoAt(warehouseId: Long, itemId: Long, asOf: Long): List<LotBalanceRow>
