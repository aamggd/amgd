...
fun RiskControlScreen(container: AppContainer, user: UserEntity, modifier: Modifier = Modifier) {
    ...
    val overdueExceptions by container.db.riskControlDao().observeOverdueExceptionCount(com.fush.erp.domain.TrustedTimeService.now()).collectAsState(initial = 0)
    ...
    if (showRisk) AddRiskDialog(onDismiss = { showRisk = false }) { title, category, description, likelihood, impact, mitigation, owner, dueDays ->
        scope.launch {
            try {
                val dueAt = dueDays?.let { com.fush.erp.domain.TrustedTimeService.now() + it * 86_400_000L }
...
private fun AddControlDialog(risks: List<RiskEntity>, onDismiss: () -> Unit, onSave: (String,String,String,String,Long?,String,String) -> Unit) {
    ...
    var selectedRisk by remember { mutableStateOf<RiskEntity?>(null) }; var design by remember { mutableStateOf("") }; var evidence by remember { mutableStateOf("") }
    AlertDialog(onDismissRequest=onDismiss, title={Text("ضابط رقابي جديد")}, text={ FushDialogForm {
        ...
        FushSearchableSelectionField(
            label = "ربط بخطر — اختياري",
            selectedText = selectedRisk?.let { "${it.riskNo} — ${it.title}" }.orEmpty(),
            options = risks,
            optionText = { "${it.riskNo} — ${it.title}" },
            searchTerms = { listOf(it.riskNo, it.title, it.category, it.ownerRole) },
            onCleared = { selectedRisk = null },
            allowClear = true,
            onClearSelected = { selectedRisk = null },
            onSelected = { selectedRisk = it },
            placeholder = "اكتب رقم أو عنوان الخطر",
        )
...
private fun AddControlTestDialog(controls: List<InternalControlEntity>, onDismiss: () -> Unit, onSave: (Long,String,String,String,String,Long?) -> Unit) {
    var control by remember { mutableStateOf(controls.firstOrNull()) }; var result by remember { mutableStateOf("PASS") }
    ...
    AlertDialog(onDismissRequest=onDismiss, title={Text("اختبار ضابط")}, text={ FushDialogForm {
        FushSearchableSelectionField(
            label = "الضابط الرقابي",
            selectedText = control?.let { "${it.controlCode} — ${it.title}" }.orEmpty(),
            options = controls,
            optionText = { "${it.controlCode} — ${it.title}" },
            searchTerms = { listOf(it.controlCode, it.title, it.controlType, it.ownerRole) },
            onCleared = { control = null },
            onSelected = { control = it },
            placeholder = "اكتب كود أو اسم الضابط",
        )
