            ...
            ElevatedCard(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    FushSearchableSelectionField(
                        ...
                        selectedText = selectedItem?.let { "${it.code} — ${it.nameAr}" }.orEmpty(),
                        options = products,
                        optionText = { "${it.code} — ${it.nameAr}" },
                        searchTerms = { listOf(it.code, it.nameAr, it.nameEn) },
                        onCleared = { selectedItemId = null; message = null },
                        onSelected = { selectedItemId = it.id; message = null },
                        placeholder = "اكتب كود أو اسم المنتج",
                    )

                    FushSearchableSelectionField(
                        label = "المحافظة",
                        selectedText = selectedPolicy?.nameAr.orEmpty(),
                        options = policies,
                        optionText = { it.nameAr },
                        searchTerms = { listOf(it.code, it.nameAr) },
                        onCleared = { selectedProvinceCode = null; message = null },
                        onSelected = { selectedProvinceCode = it.code; message = null },
                        placeholder = "اكتب اسم أو كود المحافظة",
                    )
