...
import com.fush.erp.domain.AuditTrailPresentation
...
fun GovernanceScreen(container: AppContainer, user: UserEntity, modifier: Modifier = Modifier) {
    ...
    var auditRows by remember { mutableStateOf<List<AuditTrailRow>>(emptyList()) }
    var auditLoading by remember { mutableStateOf(false) }
    var auditHasMore by remember { mutableStateOf(false) }
    var auditFrom by remember { mutableStateOf("") }
    var auditTo by remember { mutableStateOf("") }
    var auditUser by remember { mutableStateOf<UserEntity?>(null) }
    var auditSection by remember { mutableStateOf("") }
    var auditOperation by remember { mutableStateOf("") }
    var auditEventType by remember { mutableStateOf("") }
    var auditSearch by remember { mutableStateOf("") }
    var selectedAudit by remember { mutableStateOf<AuditTrailRow?>(null) }

    suspend fun loadAudit(reset: Boolean) {
        if (!canAudit || auditLoading) return
        auditLoading = true
        try {
            val offset = if (reset) 0 else auditRows.size
            val page = container.db.governanceDao().auditTrailPage(
                fromAt = AuditTrailPresentation.parseDayStart(auditFrom),
                toAt = AuditTrailPresentation.parseDayEnd(auditTo),
                userId = auditUser?.id,
            ...
            auditRows = if (reset) page else auditRows + page
            auditHasMore = page.size == 100
        } finally {
            auditLoading = false
        }
    }

    LaunchedEffect(tab, canAudit, auditFrom, auditTo, auditUser?.id, auditSection, auditOperation, auditEventType, auditSearch) {
        if (tab == 3 && canAudit) {
            delay(250)
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    FushMetricCard("أحداث التدقيق", auditRows.size.toString(), Modifier.weight(1f), helper = "الأحداث المحملة حسب الفلاتر")
                }
            }
        }
        item {
        ...
        when (tab) {
            ...
            2 -> {
                ...
                if (canAudit) {
                    item {
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            FushInlineState("سجل تدقيق غير قابل للتعديل أو الحذف. يتم تحميل 100 حدث في كل صفحة مع فلترة داخل قاعدة البيانات.", tone = FushStatusTone.Info)
                            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                FushDateField(auditFrom, { auditFrom = it }, "من تاريخ", modifier = Modifier.weight(1f), optional = true)
                                FushDateField(auditTo, { auditTo = it }, "إلى تاريخ", modifier = Modifier.weight(1f), optional = true)
                            }
                            FushSearchableSelectionField(
                                label = "المستخدم",
                                selectedText = auditUser?.let { "${it.displayName} — ${it.username}" }.orEmpty(),
                                options = users,
                                optionText = { "${it.displayName} — ${it.username}" },
                                searchTerms = { listOf(it.displayName, it.username, it.id.toString()) },
                                onSelected = { auditUser = it },
                                onCleared = { auditUser = null },
                                allowClear = true,
                                onClearSelected = { auditUser = null },
                                placeholder = "كل المستخدمين",
                            )
                            StringSelectionField(
                                "القسم",
                                AuditTrailPresentation.sections.firstOrNull { it.first == auditSection }?.second ?: "الكل",
                                AuditTrailPresentation.sections.map { it.second },
                            ) { label -> auditSection = AuditTrailPresentation.sections.first { it.second == label }.first }
                            StringSelectionField(
                                "نوع العملية",
                                AuditTrailPresentation.operations.firstOrNull { it.first == auditOperation }?.second ?: "الكل",
                                AuditTrailPresentation.operations.map { it.second },
                            ) { label -> auditOperation = AuditTrailPresentation.operations.first { it.second == label }.first }
                            StringSelectionField(
                                "نوع الحدث",
                                AuditTrailPresentation.eventTypes.firstOrNull { it.first == auditEventType }?.second ?: "الكل",
                                AuditTrailPresentation.eventTypes.map { it.second },
                            ) { label -> auditEventType = AuditTrailPresentation.eventTypes.first { it.second == label }.first }
                            OutlinedTextField(
                                value = auditSearch,
                                onValueChange = { auditSearch = it },
                                label = { Text("بحث برقم المستند / الاسم / الكود") },
                                singleLine = true,
                                modifier = Modifier.fillMaxWidth(),
                            )
                            LazyRow(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                                items(AuditTrailPresentation.quickFilters, key = { it.first.ifBlank { "ALL" } }) { filter ->
                                    FilterChip(
                                        selected = auditOperation == filter.first,
                                        onClick = { auditOperation = filter.first },
                    ...
                    if (!auditLoading && auditRows.isEmpty()) item {
                        FushEmptyState("لا توجد أحداث مطابقة", "غيّر الفلاتر أو البحث لعرض أحداث أخرى.")
                    }
                    items(auditRows, key = { "governance-audit-${it.id}" }) { e ->
                        Card(
                            modifier = Modifier.fillMaxWidth().clickable { selectedAudit = e },
                        ) {
                            Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                Text(AuditTrailPresentation.title(e), style = MaterialTheme.typography.titleMedium)
                                Text("المستخدم: ${AuditTrailPresentation.actor(e)}", style = MaterialTheme.typography.bodyMedium)
                                Text("التاريخ: ${AuditTrailPresentation.dateText(e.eventAt)}", style = MaterialTheme.typography.bodySmall)
                                Text("الوقت: ${AuditTrailPresentation.timeText(e.eventAt)}", style = MaterialTheme.typography.bodySmall)
                                Text("القسم: ${AuditTrailPresentation.sectionLabel(e.sectionCode)}", style = MaterialTheme.typography.bodySmall)
                                if (e.referenceDisplay.isNotBlank()) Text("السجل: ${e.referenceDisplay}", style = MaterialTheme.typography.bodySmall)
                                Text("العملية: ${AuditTrailPresentation.actionLabel(e.action)}", style = MaterialTheme.typography.bodySmall)
                                if (e.oldValue.isNotBlank()) Text("قبل: ${e.oldValue}", style = MaterialTheme.typography.bodySmall)
                                if (e.newValue.isNotBlank()) Text("بعد: ${e.newValue}", style = MaterialTheme.typography.bodySmall)
                                if (e.reason.isNotBlank()) Text("السبب: ${e.reason}", style = MaterialTheme.typography.bodySmall)
                            }
                        }
                    }
                    if (auditLoading) item { LinearProgressIndicator(Modifier.fillMaxWidth()) }
                    if (!auditLoading && auditHasMore) item {
                        OutlinedButton(onClick = { scope.launch { loadAudit(reset = false) } }, modifier = Modifier.fillMaxWidth()) { Text("تحميل 100 حدث إضافي") }
                    }
                }
            }
        }
    ...
    selectedAudit?.let { e ->
        AlertDialog(
            ...
            title = { Text("تفاصيل سجل التدقيق") },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    Text("المستخدم: ${AuditTrailPresentation.actor(e)}")
                    if (!e.actorUsername.isNullOrBlank()) Text("Username: ${e.actorUsername}")
                    Text("التاريخ: ${AuditTrailPresentation.dateText(e.eventAt)}")
                    Text("الوقت: ${AuditTrailPresentation.timeText(e.eventAt)}")
                    Text("نوع العملية: ${AuditTrailPresentation.actionLabel(e.action)}")
                    Text("الوحدة/القسم: ${AuditTrailPresentation.sectionLabel(e.sectionCode)}")
                    Text("Entity Type: ${e.entityType}")
                    Text("Entity ID: ${e.entityId}")
                    Text("رقم/اسم المستند: ${e.referenceDisplay}")
                    if (e.oldValue.isNotBlank()) Text("القيمة السابقة: ${e.oldValue}")
                    if (e.newValue.isNotBlank()) Text("القيمة الجديدة: ${e.newValue}")
                    if (e.reason.isNotBlank()) Text("السبب: ${e.reason}")
                    if (e.deviceInfo.isNotBlank()) Text("Reference/Device: ${e.deviceInfo}")
                    Text("الكود التقني الأصلي: ${e.action}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            },
            confirmButton = { TextButton(onClick = { selectedAudit = null }) { Text("إغلاق") } },
        )
    }
    ...
    if (showChange) AddChangeDialog(onDismiss = { showChange = false }) { type, subject, reason, quality, financial, inventory ->
        scope.launch {
            try {
                container.securityService.requirePermission(user.id, SecurityPermissions.GOVERNANCE_MANAGE)
                val no = "CHG-${com.fush.erp.domain.TrustedTimeService.now().toString().takeLast(8)}"
...
private fun AddDocumentDialog(onDismiss: () -> Unit, onSave: (String,String,String,String,String) -> Unit) {
    ...
    AlertDialog(onDismissRequest=onDismiss, title={Text("وثيقة مضبوطة جديدة")}, text={ com.fush.erp.ui.FushDialogForm {
...
private fun AddChangeDialog(onDismiss: () -> Unit, onSave: (String,String,String,String,String,String) -> Unit) {
    ...
    AlertDialog(onDismissRequest=onDismiss, title={Text("طلب تغيير")}, text={ com.fush.erp.ui.FushDialogForm {
