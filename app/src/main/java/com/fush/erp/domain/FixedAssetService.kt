...
class FixedAssetService(private val db: FushDatabase) {
    ...
    suspend fun registerAsset(request: RegisterRequest): Long = db.withTransaction {
        db.requireUserPermission(request.createdBy, SecurityPermissions.FIXED_ASSET_POST)
    ...
    suspend fun postDepreciationForPeriod(periodId: Long, createdBy: Long): List<Long> = db.withTransaction {
        db.requireUserPermission(createdBy, SecurityPermissions.FIXED_ASSET_POST)
        ...
        require(period.endDate < TrustedTimeService.requireTrustedNow()) { "لا يمكن ترحيل إهلاك فترة لم تنته بعد" }
    ...
    suspend fun reverseDepreciation(depreciationId: Long, reason: String, reversedBy: Long, reversalDate: Long = com.fush.erp.domain.TrustedTimeService.now()): Long = db.withTransaction {
        db.requireUserPermission(reversedBy, SecurityPermissions.FIXED_ASSET_POST)
        ...
        if (asset.status == "FULLY_DEPRECIATED") db.fixedAssetDao().updateAsset(asset.copy(status = "ACTIVE", updatedAt = com.fush.erp.domain.TrustedTimeService.now()))
    ...
    suspend fun disposeAsset(request: DisposalRequest): Long = db.withTransaction {
        ...
        db.fixedAssetDao().updateAsset(asset.copy(status = "DISPOSED", disposalDate = request.disposalDate, disposalProceedsBase = proceedsBase, disposalGainLossBase = gainLoss, disposalJournalEntryId = entryId, disposalReason = request.reason.trim(), updatedAt = com.fush.erp.domain.TrustedTimeService.now()))
    ...
    suspend fun reverseDisposal(disposalId: Long, reason: String, reversedBy: Long, reversalDate: Long = com.fush.erp.domain.TrustedTimeService.now()): Long = db.withTransaction {
        db.requireUserPermission(reversedBy, SecurityPermissions.FIXED_ASSET_POST)
        ...
        db.fixedAssetDao().updateAsset(asset.copy(status = if (fully) "FULLY_DEPRECIATED" else "ACTIVE", disposalDate = null, disposalProceedsBase = 0.0, disposalGainLossBase = 0.0, disposalJournalEntryId = null, disposalReason = "", updatedAt = com.fush.erp.domain.TrustedTimeService.now()))
        audit(reversedBy, "REVERSE_DISPOSAL", "FIXED_ASSET", asset.id, "journal=$reversalId", reason.trim())
        reversalId
    }

    suspend fun cancelAcquisition(assetId: Long, reason: String, cancelledBy: Long, reversalDate: Long = com.fush.erp.domain.TrustedTimeService.now()): Long = db.withTransaction {
        db.requireUserPermission(cancelledBy, SecurityPermissions.FIXED_ASSET_POST)
        ...
        db.fixedAssetDao().updateAsset(asset.copy(status = "CANCELLED", acquisitionReversalEntryId = reversalId, cancellationReason = reason.trim(), cancelledBy = cancelledBy, cancelledAt = reversalDate, updatedAt = com.fush.erp.domain.TrustedTimeService.now()))
    ...
    private suspend fun postDepreciationInternal(asset: FixedAssetEntity, fiscalYear: Int, periodNo: Int, periodStart: Long, periodEnd: Long, postingDate: Long, createdBy: Long): Long? {
        ...
        if (fully && asset.status == "ACTIVE") db.fixedAssetDao().updateAsset(asset.copy(status = "FULLY_DEPRECIATED", updatedAt = com.fush.erp.domain.TrustedTimeService.now()))
