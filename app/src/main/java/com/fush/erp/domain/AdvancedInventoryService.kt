...
class AdvancedInventoryService(private val db: FushDatabase) {
    suspend fun startCount(
        ...
        createdBy: Long,
        notes: String = "",
        countDate: Long = TrustedTimeService.now()
    ): Long = db.withTransaction {
        db.requireUserPermission(createdBy, SecurityPermissions.INVENTORY_COUNT)
        FutureDocumentDatePolicy.requireNotFuture(countDate, "تاريخ الجرد")
        require(db.warehouseDao().allActive().any { it.id == warehouseId }) { "المخزن غير موجود" }
        val effectiveDate = BusinessDatePolicy.endOfBusinessDay(countDate)
        ...
        val snapshot = db.advancedInventoryDao().snapshotAt(warehouseId, effectiveDate)
    ...
    suspend fun postCount(countId: Long, createdBy: Long): Long? = db.withTransaction {
        ...
        FutureDocumentDatePolicy.requireNotFuture(count.countDate, "تاريخ الجرد")
        ...
        if (lines.any { abs(it.varianceQtyBase) > InventoryMath.EPS }) {
            AccountingService(db).requirePostingPeriodOpen(count.countDate)
        }

        var positiveValue = 0.0
        var negativeValue = 0.0
        val effectiveDate = count.countDate
        val postedAt = TrustedTimeService.now()
        lines.forEach { line ->
            val variance = line.varianceQtyBase
            if (abs(variance) <= InventoryMath.EPS) return@forEach
            if (variance < -InventoryMath.EPS) {
                requireHistoricalLotOutflowAvailable(
                    warehouseId = count.warehouseId,
                    itemId = line.itemId,
                    lotNo = line.lotNo,
                    expiryDate = line.expiryDate,
                    movementDate = effectiveDate,
                    requestedQtyBase = -variance
                )
            }
        ...
        db.advancedInventoryDao().updateCount(count.copy(status = "POSTED", postedAt = postedAt))
        entryId
    }
    ...
    suspend fun startWarehouseTransfer(
        ...
        FutureDocumentDatePolicy.requireNotFuture(transferDate, "تاريخ التحويل")
    ...
    suspend fun addWarehouseTransferLine(
        ...
        val sourceLot = db.stockDao().lotBalancesAt(transfer.fromWarehouseId, itemId, BusinessDatePolicy.endOfBusinessDay(transfer.transferDate)).firstOrNull {
    ...
    suspend fun postWarehouseTransfer(transferId: Long, postedBy: Long): Unit = db.withTransaction {
        ...
        val prepared = lines.map { line ->
            val sourceLot = db.stockDao().lotBalancesAt(transfer.fromWarehouseId, line.itemId, BusinessDatePolicy.endOfBusinessDay(transfer.transferDate)).firstOrNull {
            ...
            if (destinationControl == null && p.control != null) {
                db.advancedInventoryDao().upsertLotControl(
                    p.control.copy(
                        ...
                        changedBy = postedBy,
                        changedAt = com.fush.erp.domain.TrustedTimeService.now()
                    )
                )
            }
        }
    ...
    suspend fun reverseWarehouseTransfer(transferId: Long, reason: String, userId: Long) = db.withTransaction {
        ...
        val reversalDate = maxOf(com.fush.erp.domain.TrustedTimeService.now(), transfer.transferDate)
        lines.forEach { line ->
    ...
    /**
     * Fail-closed guard for a stock outflow posted at a historical date.
     * The requested quantity must be available at the posting date AND must not
     * make the same lot/expiry negative at any later point in the stock ledger.
     */
    suspend fun requireHistoricalLotOutflowAvailable(
        warehouseId: Long,
        itemId: Long,
        lotNo: String?,
        expiryDate: Long?,
        ...
        warehouseId = warehouseId,
        itemId = itemId,
        lotKey = InventoryMath.lotKey(lotNo),
        expiryKey = InventoryMath.expiryKey(expiryDate),
        movementDate = movementDate,
        requestedQtyBase = requestedQtyBase
    )

    /**
     * Returns the maximum extra outflow that can be posted on the selected
     * business day without making this lot negative on that day or any later day.
     * This is a capacity query: it never fails merely because a caller wants more.
     */
    suspend fun historicalSafeLotOutflowQty(
        warehouseId: Long,
        itemId: Long,
        lotNo: String?,
        expiryDate: Long?,
        ...
        require(movementDate > 0L) { "تاريخ حركة المخزون غير صالح" }
        val timeline = db.stockDao().lotMovementTimeline(
            warehouseId,
            itemId,
            InventoryMath.lotKey(lotNo),
            InventoryMath.expiryKey(expiryDate)
        ).map { WarehouseTransferBalancePoint(it.movementDate, it.quantityBase) }
        val minimumAvailable = WarehouseTransferMath.minimumClosingBalanceFromBusinessDay(timeline, movementDate)
        return maxOf(0.0, minimumAvailable)
    }

    private suspend fun requireHistoricalLotOutflowAvailable(
        warehouseId: Long,
        itemId: Long,
        lotKey: String,
        expiryKey: Long,
    ...
    suspend fun usableLots(warehouseId: Long, itemId: Long, at: Long = com.fush.erp.domain.TrustedTimeService.now()): List<LotBalanceRow> {
        require(at > 0L) { "تاريخ رصيد المخزون غير صالح" }
        return db.stockDao().lotBalancesAt(warehouseId, itemId, at).filter { lot ->
    ...
    suspend fun usableBalance(warehouseId: Long, itemId: Long, at: Long = com.fush.erp.domain.TrustedTimeService.now()): Double =
