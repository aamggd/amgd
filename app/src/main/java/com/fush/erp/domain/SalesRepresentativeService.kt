...
class SalesRepresentativeService(private val db: FushDatabase) {
    ...
    suspend fun create(
        ...
        require(freeQtyLimitPct.isFinite() && freeQtyLimitPct in 0.0..100.0) { "حد الكمية المجانية يجب أن يكون بين 0 و100%" }
        ...
        val row = SalesRepresentativeEntity(
            ...
            freeQtyLimitPct = freeQtyLimitPct,
            notes = notes.trim(),
            createdBy = createdBy
        )
        ...
        db.governanceDao().insertAudit(
            AuditEventEntity(
                ...
                newValue = "$code|${row.fullNameAr}|${row.repType}|${row.territory}|commission=${row.commissionRatePct}|freeLimit=${row.freeQtyLimitPct}",
    ...
    suspend fun update(
        ...
        db.governanceDao().insertAudit(
            AuditEventEntity(
                ...
                entityType = "SALES_REP",
                entityId = id.toString(),
                oldValue = "${old.fullNameAr}|${old.territory}|commission=${old.commissionRatePct}|freeLimit=${old.freeQtyLimitPct}|${old.status}",
                newValue = "${row.fullNameAr}|${row.territory}|commission=${row.commissionRatePct}|freeLimit=${row.freeQtyLimitPct}|${row.status}",
