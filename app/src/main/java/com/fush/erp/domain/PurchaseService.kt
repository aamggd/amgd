...
class PurchaseService(
    ...
    data class PostPurchaseRequest(
        ...
        val freightOriginal: Double = 0.0,
        val customsOriginal: Double = 0.0,
        val otherChargesOriginal: Double = 0.0,
        val notes: String = "",
    ...
    suspend fun postPurchase(request: PostPurchaseRequest): Long = db.withTransaction {
        ...
        FutureDocumentDatePolicy.requireNotFuture(request.invoiceDate, "تاريخ فاتورة الشراء")
        ...
        val subtotalOriginal = PurchaseMath.totalOriginal(request.lines)
        val totalOriginal = PurchaseMath.invoiceTotalOriginal(
            subtotalOriginal = subtotalOriginal,
            discountOriginal = request.discountOriginal,
            freightOriginal = request.freightOriginal,
            customsOriginal = request.customsOriginal,
            otherChargesOriginal = request.otherChargesOriginal
        )
        val totalBase = PurchaseMath.toBaseAmount(totalOriginal, request.exchangeRate)
        var invoiceNo: String? = null
        var attempts = 0
        while (invoiceNo == null && attempts < 50) {
            attempts++
            val candidate = numbering.nextDocumentNo("PINV", request.invoiceDate)
            when (val reservation = documentNumberGuard.reserve(request.createdBy, "PURCHASE_INVOICE", candidate)) {
                is DocumentNumberReservationResult.Reserved, DocumentNumberReservationResult.LocalOnly -> invoiceNo = candidate
                is DocumentNumberReservationResult.InUse -> Unit // advance the local sequence and try the next company-safe number
                is DocumentNumberReservationResult.Failure -> error("تعذر حجز رقم فاتورة الشراء سحابياً: ${reservation.message}")
            }
        }
        val reservedInvoiceNo = requireNotNull(invoiceNo) { "تعذر إيجاد رقم فاتورة شراء غير مستخدم بعد 50 محاولة" }
        ...
        request.lines.forEach { line ->
            ...
            if (item.lotTracked) require(!line.lotNo.isNullOrBlank()) { "الصنف ${item.nameAr} يتطلب رقم تشغيلة عند الشراء" }
            if (item.expiryTracked) require(line.expiryDate != null && line.expiryDate > 0L) { "الصنف ${item.nameAr} يتطلب تاريخ صلاحية عند الشراء" }
            ...
            val unitCostBase = PurchaseMath.allocatedUnitCostBase(
                line = line,
                exchangeRate = request.exchangeRate,
                subtotalOriginal = subtotalOriginal,
                invoiceTotalOriginal = totalOriginal
            )
        ...
        postPurchaseJournal(invoiceId, reservedInvoiceNo, totalBase, request, cashTreasury?.accountId)
        db.governanceDao().insertAudit(
            AuditEventEntity(
                ...
                newValue = "invoiceNo=$reservedInvoiceNo|supplier=${supplier.code}|paymentType=${request.paymentType}|subtotal=$subtotalOriginal|discount=${request.discountOriginal}|freight=${request.freightOriginal}|customs=${request.customsOriginal}|other=${request.otherChargesOriginal}|totalBase=$totalBase|lines=${request.lines.size}",
                reason = request.notes.trim().ifBlank { "ترحيل فاتورة شراء" }
            )
        )
        invoiceId
    }
    ...
    data class PostPurchaseReturnRequest(
        ...
        val reason: String,
        val createdBy: Long,
        val returnDate: Long = com.fush.erp.domain.TrustedTimeService.now(),
    ...
    suspend fun postPurchaseReturn(request: PostPurchaseReturnRequest): Long = db.withTransaction {
        ...
        FutureDocumentDatePolicy.requireNotFuture(request.returnDate, "تاريخ مرتجع الشراء")
        ...
        TransactionChronology.requireOnOrAfter(
            ...
            eventLabel = "تاريخ مرتجع الشراء",
            sourceLabel = "تاريخ فاتورة الشراء ${invoice.invoiceNo}",
        )
        ...
        val prepared = request.lines.map { draft ->
            ...
            val returnBaseQuantity = draft.quantity * line.factorToBase
            PreparedPurchaseReturnLine(
                line = line,
                quantity = draft.quantity,
                baseQuantity = returnBaseQuantity,
                totalOriginal = PurchaseMath.returnTotalOriginal(returnBaseQuantity, line.unitCostBase, invoice.exchangeRate)
            )
        }

        // Validate the exact original lot/expiry in aggregate at the return date.
        // For backdated returns, also guarantee that the inserted outflow cannot make
        // this lot negative at any later point in the stock ledger.
        ...
        prepared.groupBy { Triple(it.line.itemId, it.line.lotNo?.trim().orEmpty(), it.line.expiryDate ?: -1L) }
            .forEach { (key, selectedLines) ->
                ...
                val historicalLot = db.stockDao().lotBalancesAt(
                    warehouseId = invoice.warehouseId,
                    itemId = itemId,
                    asOf = BusinessDatePolicy.endOfBusinessDay(request.returnDate)
                ).firstOrNull {
                    InventoryMath.lotKey(it.lotNo) == lotKey && InventoryMath.expiryKey(it.expiryDate) == expiryKey
                }
                require(historicalLot != null && historicalLot.quantityBase + 1e-9 >= requiredBase) {
                    "المخزون المتاح من التشغيلة الأصلية في تاريخ المرتجع لا يكفي لتنفيذ المرتجع"
                }
                advancedInventory.requireHistoricalLotOutflowAvailable(
                    warehouseId = invoice.warehouseId,
                    itemId = itemId,
                    lotNo = selectedLines.first().line.lotNo,
                    expiryDate = selectedLines.first().line.expiryDate,
                    movementDate = request.returnDate,
                    requestedQtyBase = requiredBase
                )
            }
        ...
        if (request.settlementType == "CASH_REFUND") {
            requirePurchaseCashRefundTimelineCovered(
                invoice = invoice,
        ...
        prepared.forEach { preparedLine ->
            ...
            db.purchaseDao().insertReturnLine(
                PurchaseReturnLineEntity(
                    ...
                    unitPriceOriginal = if (preparedLine.quantity <= 1e-12) 0.0 else preparedLine.totalOriginal / preparedLine.quantity,
        ...
        db.governanceDao().insertAudit(
            AuditEventEntity(
                ...
                newValue = "returnNo=$returnNo|settlement=${request.settlementType}|totalBase=$totalBase|lines=${prepared.size}",
                reason = request.reason.trim()
            )
        )
        returnId
    ...
    suspend fun postSupplierPayment(
        ...
        notes: String,
        createdBy: Long,
        paymentDate: Long = com.fush.erp.domain.TrustedTimeService.now()
    ...
    suspend fun postSupplierPaymentAutoAllocate(
        ...
        FutureDocumentDatePolicy.requireNotFuture(paymentDate, "تاريخ دفعة المورد")
        val open = db.purchaseDao().openSupplierInvoicesAsOf(supplierId, paymentDate)
    ...
    private suspend fun postSupplierPaymentAllocationsInternal(
        ...
        val prepared = allocations.map { allocation ->
            ...
            TransactionChronology.requireOnOrAfter(
                ...
                sourceDate = invoice.invoiceDate,
                eventLabel = "تاريخ دفعة المورد",
                sourceLabel = "تاريخ فاتورة المورد ${invoice.invoiceNo}",
            )
            val returnedCreditBase = db.purchaseDao().supplierCreditReturnedBaseForInvoiceAsOf(invoice.id, paymentDate)
            val alreadyPaidBase = db.purchaseDao().paidBaseForInvoiceAsOf(invoice.id, paymentDate)
            val outstandingBase = SupplierApMath.outstandingBase(invoice.totalBase, returnedCreditBase, alreadyPaidBase)
            require(outstandingBase > 1e-8) { "لا يوجد رصيد مستحق على الفاتورة ${invoice.invoiceNo} في تاريخ الدفع" }
            val split = SupplierApMath.paymentSplit(allocation.amountOriginal, invoice.exchangeRate, paymentExchangeRate)
            require(split.allocatedBase <= outstandingBase + 1e-8) { "الدفعة تتجاوز الرصيد المتبقي على الفاتورة ${invoice.invoiceNo} في تاريخ الدفع" }
            val allTimeReturnedCreditBase = db.purchaseDao().returnsForInvoice(invoice.id)
                .filter { it.status == "POSTED" && it.settlementType == "SUPPLIER_CREDIT" }
                .sumOf { it.totalBase }
            val allTimePaidBase = db.purchaseDao().paidBaseForInvoice(invoice.id)
            val currentOutstandingBase = SupplierApMath.outstandingBase(invoice.totalBase, allTimeReturnedCreditBase, allTimePaidBase)
            require(split.allocatedBase <= currentOutstandingBase + 1e-8) {
                "الدفعة التاريخية ستجعل فاتورة المورد ${invoice.invoiceNo} مفرطة التخصيص في تاريخ لاحق؛ اعكس/أعد تخصيص الدفعات اللاحقة أولاً"
            }
        ...
        db.governanceDao().insertAudit(
            AuditEventEntity(
                ...
                newValue = "paymentNo=$paymentNo|supplier=${supplier.code}|cashBase=$totalCashBase|allocatedBase=$totalAllocatedBase|invoices=${prepared.joinToString(",") { it.invoice.invoiceNo }}",
                reason = notes.trim().ifBlank { "ترحيل دفعة مورد" }
            )
        )
    ...
    suspend fun reverseSupplierPayment(
        ...
        db.requireUserPermission(createdBy, SecurityPermissions.SUPPLIER_PAYMENT_POST)
        FutureDocumentDatePolicy.requireNotFuture(reversalDate, "تاريخ عكس دفعة المورد")
        ...
        allocations.forEach { allocation ->
            ...
            requirePurchaseCashRefundTimelineCovered(
                invoice = invoice,
                proposedCashDeltaBase = -allocation.allocatedBase,
    ...
    private suspend fun requirePurchaseCashRefundTimelineCovered(
        invoice: PurchaseInvoiceEntity,
        ...
        val relevantDates = buildSet {
            add(eventDate)
            db.purchaseDao().supplierPaymentsForInvoice(invoice.id)
                .filter { it.paymentDate >= eventDate }
                .forEach { add(it.paymentDate) }
            db.purchaseDao().returnsForInvoice(invoice.id)
        ...
        for (date in relevantDates) {
            val actualPaidBase = if (invoice.paymentType == "CASH") {
                invoice.totalBase
            } else {
                db.purchaseDao().paidBaseForInvoiceAsOf(invoice.id, date)
            } + proposedCashDeltaBase
            val refundedBase = db.purchaseDao().cashRefundedBaseForInvoiceAsOf(invoice.id, date)
            CashRefundCoveragePolicy.requireCovered(
                actualCashBase = actualPaidBase,
                alreadyRefundedBase = refundedBase,
                requestedRefundBase = proposedRefundBase,
                context = "فاتورة الشراء ${invoice.invoiceNo}: استرداد المورد النقدي يجب أن يكون مغطى بالنقد المدفوع فعلياً"
            )
        }
    }
