...
class MasterDataService(private val db: FushDatabase) {
    ...
    suspend fun createItem(
        ...
        if (category == "FINISHED_GOOD") {
            require(shelfLifeDays != null && shelfLifeDays > 0) { "مدة صلاحية المنتج النهائي إلزامية" }
        }
        ...
        val effectiveShelfLife = shelfLifeDays
        val row = ItemEntity(
            code = code,
            nameAr = nameAr.trim(),
            nameEn = nameEn.trim(),
    ...
    suspend fun updateItem(
        ...
        val effectiveShelfLife = if (old.category == "FINISHED_GOOD") (shelfLifeDays ?: old.shelfLifeDays) else shelfLifeDays
        if (old.category == "FINISHED_GOOD") {
            require(effectiveShelfLife != null && effectiveShelfLife > 0) { "مدة صلاحية المنتج النهائي إلزامية" }
        }
        val row = old.copy(
            nameAr = nameAr.trim(),
            nameEn = nameEn.trim(),
