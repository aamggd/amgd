...
class AccountingService(private val db: FushDatabase) {
    ...
    data class VoucherRequest(
        ...
        val referenceNo: String = "",
        val voucherDate: Long = com.fush.erp.domain.TrustedTimeService.now(),
        val createdBy: Long,
    ...
    suspend fun addTreasuryAccountWithNewLedger(
        code: String,
        nameAr: String,
        kind: String,
        ledgerCode: String,
        ...
        require(code.trim().isNotBlank() && nameAr.trim().isNotBlank()) { "كود واسم الخزينة مطلوبان" }
        require(kind in setOf("CASH", "BANK")) { "نوع الخزينة غير صالح" }
        require(ledgerCode.trim().isNotBlank() && ledgerNameAr.trim().isNotBlank()) { "كود واسم حساب الأستاذ الجديد مطلوبان" }
        require(db.accountDao().byCode(ledgerCode.trim()) == null) { "كود حساب الأستاذ مستخدم مسبقاً" }
        require(db.currencyDao().allActive().any { it.code == currencyCode }) { "العملة غير موجودة" }

        val assetsRoot = requireNotNull(db.accountDao().byCode("1000")) { "حساب الأصول الرئيسي 1000 غير موجود" }
        val accountId = db.accountDao().insert(
            AccountEntity(
                code = ledgerCode.trim(),
                nameAr = ledgerNameAr.trim(),
                nameEn = ledgerNameAr.trim(),
                type = "ASSET",
                parentCode = assetsRoot.code,
                isPosting = true,
                isActive = true
            )
        )
    ...
    suspend fun updateTreasuryGroup(
        ...
        db.requireUserPermission(updatedBy, SecurityPermissions.TREASURY_POST)
        require(nameAr.trim().isNotBlank()) { "اسم الخزينة مطلوب" }
        require(kind in setOf("CASH", "BANK")) { "نوع الخزينة غير صالح" }
        val anchor = requireNotNull(db.accountingDao().treasuryById(treasuryAccountId)) { "الخزينة غير موجودة" }
        val groupCode = anchor.groupCode.ifBlank { anchor.code }
        val rows = db.accountingDao().treasuriesByGroupCode(groupCode).ifEmpty { listOf(anchor) }
        val oldValue = rows.joinToString(";") { "${it.code}|${it.nameAr}|${it.kind}|${it.currencyCode}|${it.isActive}" }
        rows.forEach { row ->
            db.accountingDao().updateTreasury(
                row.copy(
                    groupCode = groupCode,
                    nameAr = nameAr.trim(),
                    kind = kind,
                    bankName = if (kind == "BANK") bankName.trim() else "",
                    accountNumber = if (kind == "BANK") accountNumber.trim() else "",
                    isActive = isActive
                )
            )
        }
        db.governanceDao().insertAudit(
            AuditEventEntity(
                ...
                entityType = "TREASURY_GROUP",
                entityId = groupCode,
                oldValue = oldValue,
                newValue = "${nameAr.trim()}|$kind|active=$isActive|bank=${bankName.trim()}|account=${accountNumber.trim()}",
                reason = "تعديل بيانات الخزينة متعددة العملات"
            )
        )
    }

    suspend fun addCurrencyToTreasuryGroup(
        treasuryAccountId: Long,
        currencyCode: String,
        createdBy: Long
    ): Long = db.withTransaction {
        ...
        require(db.currencyDao().allActive().any { it.code == normalizedCurrency }) { "العملة غير موجودة أو غير نشطة" }
        val siblings = db.accountingDao().treasuriesByGroupCode(groupCode).ifEmpty { listOf(anchor) }
        require(siblings.none { it.currencyCode.equals(normalizedCurrency, ignoreCase = true) }) {
            "هذه العملة مضافة بالفعل إلى الخزينة"
        }

        val sourceLedger = requireNotNull(db.accountDao().byId(anchor.accountId)) { "حساب أستاذ الخزينة غير موجود" }
        val treasuryCodeBase = "${groupCode}-${normalizedCurrency}".replace(Regex("[^A-Za-z0-9_-]"), "-")
        val treasuryCode = uniqueTreasuryCode(treasuryCodeBase)
        val ledgerBase = "${sourceLedger.code}-${normalizedCurrency}".replace(Regex("[^A-Za-z0-9_-]"), "-")
        val ledgerCode = uniqueLedgerCode(ledgerBase)
        val accountId = db.accountDao().insert(
            AccountEntity(
                code = ledgerCode,
                nameAr = "${anchor.nameAr} - $normalizedCurrency",
                nameEn = "${anchor.nameAr} - $normalizedCurrency",
                type = "ASSET",
                parentCode = sourceLedger.parentCode ?: "1000",
                isPosting = true,
                isActive = true
            )
        )
        val id = db.accountingDao().insertTreasury(
            TreasuryAccountEntity(
                code = treasuryCode,
                groupCode = groupCode,
                nameAr = anchor.nameAr,
                kind = anchor.kind,
                accountId = accountId,
                currencyCode = normalizedCurrency,
                bankName = anchor.bankName,
        ...
        db.governanceDao().insertAudit(
            AuditEventEntity(
                userId = createdBy,
                action = "TREASURY_ADD_CURRENCY",
                entityType = "TREASURY_GROUP",
                entityId = groupCode,
                newValue = "$normalizedCurrency|treasury=$treasuryCode|ledger=$ledgerCode",
                reason = "إضافة عملة إلى نفس الخزينة مع حساب أستاذ مستقل"
            )
        )
        id
    }

    private suspend fun uniqueTreasuryCode(base: String): String {
        var candidate = base.take(48).ifBlank { "TREASURY" }
        var suffix = 2
        while (db.accountingDao().treasuryByCode(candidate) != null) {
            val tail = "-$suffix"
            candidate = base.take((48 - tail.length).coerceAtLeast(1)) + tail
            suffix++
        }
        return candidate
    }

    private suspend fun uniqueLedgerCode(base: String): String {
        var candidate = base.take(48).ifBlank { "TREASURY-LEDGER" }
        var suffix = 2
        while (db.accountDao().byCode(candidate) != null) {
            val tail = "-$suffix"
            candidate = base.take((48 - tail.length).coerceAtLeast(1)) + tail
            suffix++
        }
    ...
    suspend fun postVoucher(request: VoucherRequest): Long = db.withTransaction {
        ...
        val operationSourceId = TreasuryVoucherOperationIdentity.sourceId(request.operationId)
        ...
        // Idempotent retry: one dialog/confirmation owns one stable operationId. If the first
        // attempt already committed, return the same journal id instead of creating a second
        // voucher. Reusing the operation id with a changed payload is rejected fail-closed.
        db.journalDao().bySource(sourceType, operationSourceId)?.let { existing ->
            requireVoucherReplayMatchesExisting(existing, request, description, lines)
    ...
    suspend fun reverseEntry(entryId: Long, reason: String, createdBy: Long, reversalDate: Long = com.fush.erp.domain.TrustedTimeService.now()): Long = db.withTransaction {
        db.requireUserPermission(createdBy, SecurityPermissions.ACCOUNTING_POST)
        ...
        require(!requiresOperationalDocumentReversal(original.sourceType, linkedVoucher != null)) {
            operationalDocumentReversalMessage(original.sourceType)
                ?: "هذا القيد ناتج عن عملية تشغيلية؛ يجب عكسه من شاشة العملية الأصلية لحماية التتبع"
        }
        require(canReverseEntrySource(original.sourceType, linkedVoucher != null)) {
            "هذا القيد ناتج عن عملية تشغيلية؛ يجب عكسه من شاشة العملية الأصلية لحماية التتبع"
        }
    ...
    suspend fun createCashCount(
        ...
        db.requireUserPermission(createdBy, SecurityPermissions.CASH_COUNT_POST)
    ...
    suspend fun resolveCashCountDifference(
        ...
        db.requireUserPermission(resolvedBy, SecurityPermissions.CASH_COUNT_POST)
        db.requireUserPermission(resolvedBy, SecurityPermissions.ACCOUNTING_POST)
        ...
        db.accountingDao().updateCashCount(count.copy(
            ...
            resolvedAt = com.fush.erp.domain.TrustedTimeService.now()
        ))
        db.governanceDao().insertAudit(AuditEventEntity(
            userId = resolvedBy,
            action = "RESOLVE",
    ...
    suspend fun createBankStatement(
        ...
        db.requireUserPermission(createdBy, SecurityPermissions.BANK_RECONCILIATION_POST)
    ...
    suspend fun matchBankStatementLine(lineId: Long, journalEntryId: Long, matchedBy: Long) = db.withTransaction {
        db.requireUserPermission(matchedBy, SecurityPermissions.BANK_RECONCILIATION_POST)
        ...
        db.accountingDao().updateBankStatementLine(
            line.copy(
                ...
                matchedBy = matchedBy,
                matchedAt = com.fush.erp.domain.TrustedTimeService.now()
            )
        )
        db.governanceDao().insertAudit(
            AuditEventEntity(
    ...
    suspend fun unmatchBankStatementLine(lineId: Long, userId: Long) = db.withTransaction {
        db.requireUserPermission(userId, SecurityPermissions.BANK_RECONCILIATION_POST)
        val line = requireNotNull(db.accountingDao().bankStatementLineById(lineId)) { "حركة كشف البنك غير موجودة" }
        val statement = requireNotNull(db.accountingDao().bankStatementById(line.statementId)) { "كشف البنك غير موجود" }
        require(statement.status == "DRAFT") { "كشف البنك تمت مطابقته ولا يمكن تعديله" }
    ...
    suspend fun bankReconciliation(statementId: Long): ForeignBankReconciliationComputation {
        ...
        val matchedIds = db.accountingDao().matchedJournalIdsThrough(treasury.id, statement.endDate).toSet()
    ...
    suspend fun finalizeBankReconciliation(statementId: Long, reconciledBy: Long): ForeignBankReconciliationComputation = db.withTransaction {
        db.requireUserPermission(reconciledBy, SecurityPermissions.BANK_RECONCILIATION_POST)
        val statement = requireNotNull(db.accountingDao().bankStatementById(statementId)) { "كشف البنك غير موجود" }
        ...
        db.accountingDao().updateBankStatement(statement.copy(
            status = "RECONCILED", reconciledBy = reconciledBy, reconciledAt = com.fush.erp.domain.TrustedTimeService.now()
        ))
        db.governanceDao().insertAudit(AuditEventEntity(
            userId = reconciledBy,
            action = "RECONCILE",
    ...
    private suspend fun reverseFxRevaluationInternal(
        ...
        db.accountingDao().updateFxRevaluation(row.copy(
            ...
            reversedAt = com.fush.erp.domain.TrustedTimeService.now(),
            reversalReason = reason.trim()
        ))
        db.governanceDao().insertAudit(AuditEventEntity(
            userId = reversedBy,
    ...
    suspend fun revalueForeignTreasuries(
        ...
        db.requireUserPermission(createdBy, SecurityPermissions.FX_REVALUATION_POST)
        require(reason.trim().isNotBlank()) { "سبب إعادة تقييم العملات مطلوب" }
        require(valuationDate <= TrustedTimeService.requireTrustedNow()) { "لا يمكن إعادة تقييم العملات بتاريخ مستقبلي" }
    ...
    suspend fun treasuryControl(fromDate: Long, toDate: Long): TreasuryControlReport {
        ...
        val endDayStart = BusinessTimeZone.calendarAt(toDate).apply {
    ...
    suspend fun createCalendarFiscalYear(fiscalYear: Int, createdBy: Long): List<Long> = db.withTransaction {
        db.requireUserPermission(createdBy, SecurityPermissions.ACCOUNTING_PERIOD_MANAGE)
        ...
        val ids = mutableListOf<Long>()
        for (month in 0..11) {
            val start = Calendar.getInstance(BusinessTimeZone.timeZone).apply {
                clear()
                set(fiscalYear, month, 1, 0, 0, 0)
                set(Calendar.MILLISECOND, 0)
            }.timeInMillis
            val next = Calendar.getInstance(BusinessTimeZone.timeZone).apply {
                clear()
                set(fiscalYear, month, 1, 0, 0, 0)
    ...
    suspend fun closePeriod(periodId: Long, reason: String, closedBy: Long): AccountingReconciliationReport = db.withTransaction {
        db.requireUserPermission(closedBy, SecurityPermissions.ACCOUNTING_PERIOD_MANAGE)
        ...
        require(period.endDate < TrustedTimeService.requireTrustedNow()) { "لا يمكن إقفال فترة محاسبية لم تنته بعد" }
    ...
    suspend fun reopenPeriod(periodId: Long, reason: String, reopenedBy: Long) = db.withTransaction {
        db.requireUserPermission(reopenedBy, SecurityPermissions.ACCOUNTING_PERIOD_MANAGE)
    ...
    suspend fun closeFiscalYear(
        ...
        db.requireUserPermission(closedBy, SecurityPermissions.ACCOUNTING_YEAR_CLOSE)
        ...
        require(finalPeriod.endDate < TrustedTimeService.requireTrustedNow()) { "لا يمكن إقفال سنة مالية لم تنته بعد" }
    ...
    suspend fun reopenFiscalYear(
        ...
        db.requireUserPermission(reopenedBy, SecurityPermissions.ACCOUNTING_YEAR_CLOSE)
    ...
    suspend fun requirePostingPeriodOpen(entryDate: Long) {
        val period = db.accountingDao().periodForDate(entryDate)
    ...
    private suspend fun insertExpenseDimension(
        ...
        when (referenceType) {
            ...
            "SALES_ORDER", "PURCHASE_ORDER", "DISTRIBUTION", "SHIPMENT", "BRANCH", "FACILITY", "OTHER" -> {
    ...
    private suspend fun requireVoucherReplayMatchesExisting(
        existing: JournalEntryEntity,
        request: VoucherRequest,
        expectedDescription: String,
        expectedLines: List<DraftJournalLine>
    ) {
        require(existing.entryDate == request.voucherDate) { "TREASURY_VOUCHER_OPERATION_ID_REUSED_WITH_DIFFERENT_PAYLOAD" }
        require(existing.currencyCode == request.currencyCode) { "TREASURY_VOUCHER_OPERATION_ID_REUSED_WITH_DIFFERENT_PAYLOAD" }
        require(kotlin.math.abs(existing.exchangeRate - request.exchangeRate) <= 0.0000001) { "TREASURY_VOUCHER_OPERATION_ID_REUSED_WITH_DIFFERENT_PAYLOAD" }
        require(existing.description == expectedDescription) { "TREASURY_VOUCHER_OPERATION_ID_REUSED_WITH_DIFFERENT_PAYLOAD" }
        require(existing.createdBy == request.createdBy) { "TREASURY_VOUCHER_OPERATION_ID_REUSED_BY_DIFFERENT_USER" }

        val actualLines = db.journalDao().linesForEntry(existing.id)
        require(actualLines.size == expectedLines.size) { "TREASURY_VOUCHER_OPERATION_ID_REUSED_WITH_DIFFERENT_PAYLOAD" }
        val unmatched = actualLines.toMutableList()
        expectedLines.forEach { expected ->
            val index = unmatched.indexOfFirst { actual ->
                actual.accountId == expected.accountId &&
                    kotlin.math.abs(actual.debit - expected.debit) <= moneyTolerance &&
                    kotlin.math.abs(actual.credit - expected.credit) <= moneyTolerance
            }
            require(index >= 0) { "TREASURY_VOUCHER_OPERATION_ID_REUSED_WITH_DIFFERENT_PAYLOAD" }
    ...
    private fun documentNo(prefix: String): String {
        val stamp = SimpleDateFormat("yyyyMMdd-HHmmss", Locale.US).format(Date(TrustedTimeService.now()))
        return "$prefix-$stamp-${UUID.randomUUID().toString().take(6)}"
    }

    companion object {
        /**
         * Operational AR/AP documents own subledger state outside the general ledger.
         * They must be reversed by their owning service so both GL and subledger move together.
         * A party_voucher is the explicit exception: it has its own linked lifecycle and
         * reverseEntry() updates that linked voucher atomically.
         */
        val OPERATIONAL_DOCUMENT_REVERSAL_SOURCE_TYPES = setOf(
        ...
        fun requiresOperationalDocumentReversal(sourceType: String, hasLinkedPartyVoucher: Boolean): Boolean =
            !hasLinkedPartyVoucher && sourceType in OPERATIONAL_DOCUMENT_REVERSAL_SOURCE_TYPES

        fun canReverseEntrySource(sourceType: String, hasLinkedPartyVoucher: Boolean): Boolean =
            hasLinkedPartyVoucher || sourceType in REVERSIBLE_SOURCE_TYPES

        fun operationalDocumentReversalMessage(sourceType: String): String? = when (sourceType) {
            TreasuryMovementType.CUSTOMER_RECEIPT.sourceType ->
                "تحصيل العميل التشغيلي لا يُعكس من اليومية العامة. استخدم شاشة العميل/المبيعات لعكس التحصيل؛ وإذا كان القيد سند طرف فاعكس السند من شاشة السند."
            TreasuryMovementType.SUPPLIER_PAYMENT.sourceType ->
                "دفعة المورد التشغيلية لا تُعكس من اليومية العامة. استخدم شاشة المورد/المشتريات لعكس الدفعة؛ وإذا كان القيد سند طرف فاعكس السند من شاشة السند."
            else -> null
        }
    }
}
