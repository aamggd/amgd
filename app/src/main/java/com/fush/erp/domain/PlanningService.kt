...
class PlanningService(private val db: FushDatabase) {
    ...
    suspend fun weeklySalesActual(plan: DemandPlanEntity): List<WeeklySalesActualRow> {
        ...
        val provinceName = db.geographyDao().provincePolicy(plan.provinceCode)?.nameAr ?: plan.provinceCode
        return db.planningDao().weeklySalesActual(plan.itemId, plan.provinceCode, provinceName, fromDate, toDate)
    }
    ...
    suspend fun seasonalDemandAnalysis(
        ...
        val provinceName = db.geographyDao().provincePolicy(provinceCode)?.nameAr ?: provinceCode
        val raw = db.planningDao().monthlyDemandHistory(itemId, provinceCode, provinceName, fromDate, toDate)
