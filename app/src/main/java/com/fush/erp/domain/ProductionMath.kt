...
object ProductionMath {
    ...
    fun orderComponentQuantity(componentQty: Double, plannedOutputQty: Double, itemCode: String): Double {
        val code = itemCode.trim().uppercase()
        return when {
            code.startsWith("PK-BOTTLE-") || code.startsWith("PK-LABEL-") ->
                requireWholePieceQuantity(plannedOutputQty, "كمية الإنتاج المخططة")
            code == "PK-PACK" || code == "PK-PACK-LABEL" ->
                packagingPackCount(plannedOutputQty, 24)
            else -> fixedBatchComponentQuantity(componentQty, plannedOutputQty)
        }
    }
    ...
    fun isIssueLotTrackingComplete(
        lotTracked: Boolean,
        expiryTracked: Boolean,
        lotNo: String?,
        expiryDate: Long?
    ): Boolean = (!lotTracked || !lotNo.isNullOrBlank()) &&
        (!expiryTracked || (expiryDate != null && expiryDate > 0L))
