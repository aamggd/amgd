...
class EmployeeService(private val db: FushDatabase) {
    ...
    suspend fun recordTraining(
        ...
        val completedAt = com.fush.erp.domain.TrustedTimeService.now()
    ...
    suspend fun assignOperator(orderId: Long, employeeId: Long, assignedBy: Long) = db.withTransaction {
        ...
        if (current == null) {
            ...
            db.employeeDao().updateOperatorAssignment(current.copy(employeeId = employeeId, assignedBy = assignedBy, assignedAt = com.fush.erp.domain.TrustedTimeService.now()))
        }
        audit(assignedBy, "ASSIGN", "PRODUCTION_OPERATOR", orderId.toString(), current?.employeeId?.toString() ?: "", employeeId.toString(), "تعيين موظف إنتاج وربط استحقاق الأجور بأمر الإنتاج")
    }

    /**
     * Equipment authorization remains available as an HR/maintenance record, but
     * it no longer gates production. Production operator assignment is independent
     * from maintenance/safety readiness and authorization state.
     */
    @Suppress("UNUSED_PARAMETER")
    suspend fun assertEmployeeCanOperateAsset(employeeId: Long, assetId: Long, at: Long = com.fush.erp.domain.TrustedTimeService.now()) {
        return
    }
    ...
    private fun documentNo(prefix: String): String {
        val date = SimpleDateFormat("yyMMdd-HHmmss", Locale.US).format(Date(TrustedTimeService.now()))
