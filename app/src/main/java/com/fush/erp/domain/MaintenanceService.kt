...
class MaintenanceService(private val db: FushDatabase) {
    ...
    suspend fun openPreventiveWorkOrder(planId: Long, dueAt: Long?, createdBy: Long): Long = db.withTransaction {
        ...
        db.maintenanceDao().insertWorkOrder(
            MaintenanceWorkOrderEntity(
                ...
                planId = plan.id,
                workType = "PREVENTIVE",
                openedAt = com.fush.erp.domain.TrustedTimeService.now(),
    ...
    suspend fun reportBreakdown(
        ...
        downtimeMinutes: Int,
        createdBy: Long,
        occurredAt: Long = com.fush.erp.domain.TrustedTimeService.now()
    ): Long = db.withTransaction {
        db.requireUserPermission(createdBy, SecurityPermissions.MAINTENANCE_MANAGE)
    ...
    suspend fun reportSafetyIncident(
        ...
        db.maintenanceDao().insertSafetyIncident(
            SafetyIncidentEntity(
                ...
                occurredAt = com.fush.erp.domain.TrustedTimeService.now(),
    ...
    suspend fun recordSafetyInspection(
        ...
        db.maintenanceDao().insertSafetyInspection(
            SafetyInspectionEntity(
                ...
                inspectionDate = com.fush.erp.domain.TrustedTimeService.now(),
                area = area.trim(),
                ...
                correctiveAction = correctiveAction.trim(),
                dueAt = dueAt,
                closedAt = if (result == "PASS") com.fush.erp.domain.TrustedTimeService.now() else null,
                inspectedBy = inspectedBy
            )
        )
    }
    ...
    /**
     * Legacy bridge kept for source/API compatibility only.
     * Production no longer depends on maintenance status, due plans, calibration,
     * inspection or PRE_START readiness. Maintenance remains fully functional as
     * an independent module and can still record/report those conditions.
     */
    @Suppress("UNUSED_PARAMETER")
    suspend fun assertAssetCanOperate(assetId: Long, now: Long = com.fush.erp.domain.TrustedTimeService.now()) {
        return
    }
